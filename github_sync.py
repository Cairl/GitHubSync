...
def wrap_ansi_line(line, width):
    """将包含 ANSI 转义序列的行按指定显示宽度折行，返回行列表"""
    plain = strip_ansi(line)
    if get_display_width(plain) <= width:
        return [line]
    ...
    while i < n:
        if line[i] == '\x1b':
            ...
            if j < n and line[j] == '[':
                j += 1
                while j < n and line[j] not in 'ABCDEFGHJKSTfhimnprsu':
                    j += 1
                if j < n:
                    j += 1
            elif j < n and line[j] == ']':
        ...
        cw = get_display_width(line[i])
        if visible_pos + cw > width and visible_pos > 0:
            result.append(current)
            sgr_seqs = re.findall(r'\x1b\[[0-9;]*m', current)
            current = sgr_seqs[-1] if sgr_seqs else ""
            visible_pos = 0
...
class Colors:
    ...
    WHITE = "\033[38;2;205;214;244m"     # 亮白色 #CDD6F4
    STRIKETHROUGH = "\033[9m"
    BG_BLUE = "\033[48;2;49;116;143m"     # 选中的背景色 (蓝绿色)
    BG_RESET = "\033[49m"
...
class GitManager:
    ...
    def configure_remote(self):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        sys.stdout.write(f" {Colors.DIM}[{timestamp}]{Colors.RESET} {Colors.YELLOW}正在配置远程仓库: {Colors.RESET}")
    ...
    def get_render_lines(self):
        ...
        if status["initialized"]:
            ...
            status_lines = [
                ...
                f"分支: {Colors.WHITE}{status['branch']}{Colors.RESET}",
        ...
        if self.git.logs:
            ...
            if available_log_rows > 0:
                ...
                for log in reversed(display_logs):
                    wrapped = wrap_ansi_line(f" {log}", log_width)
                    wrapped_count += len(wrapped)
                skip_count = max(0, wrapped_count - available_log_rows)
                for log in display_logs:
                    wrapped = wrap_ansi_line(f" {log}", log_width)
                    if skip_count > 0:
                        skip_count -= len(wrapped)
                        if skip_count < 0:
                            lines.extend(wrapped[-skip_count:])
                    else:
                        lines.extend(wrapped)
            else:
                # 空间极度受限时至少显示最后一条
                for wrapped_line in wrap_ansi_line(f" {self.git.logs[-1]}", log_width):
