...
- **极简交互模式**：无子命令直接启动，顶栏常驻（项目 / 分支·状态 / 菜单），← → 切换标签页即显示内容（免 Enter）
...
导航栏固定三项：`推送` `拉取` `文件`（仅选中项带括号如 `[推送]` + 底色框选，未选中项为裸文本），用左右键循环切换标签页、切换即显示内容（免 Enter）；推送 / 拉取有新的同步时文本两侧加星号（如 `*推送*`，选中时 `[*推送*]`）。

| 按键 | 功能 |
|------|------|
| `←` `→` | 循环切换标签页（`[ ]` 框选推送 / 拉取 / 文件），切换即显示内容 |
| `Enter` | 执行当前标签内选中项（推送 / 对齐远程·恢复历史 / 切换忽略） |
| `↑` `↓` | 标签内移动（文件列表、拉取历史列表） |
...
操作结果无文字回显，由视图状态表达：推送时待推文件标记 `[·]`（上传中）→ `[✓]`（完成）/ `[✕]`（失败）；拉取标签页中与远程一致的提交 hash 标青色；文件标签页操作失败行首显示 `[!]`。
...
├── tui/                   # 交互表现层（screen / interactive / view_base / push_view / pull_view / files_view）
