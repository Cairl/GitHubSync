...
def test_push_renders_stage_progress():
    """按 Enter 推送：推送页阶段行末行为结果，非 CLI 日志。"""
    ...
    with pytest.raises(StopIteration):
        app.run()
    out = app._view
    assert "✓ Push completed (2 change(s))" in out.splitlines()[-1]  # 末行=结果
    assert "Scanning complete" in out                          # 阶段行留痕
...
def test_push_failure_renders_error():
    ...
    assert "Network error" in out.splitlines()[-1]  # 失败原因


def test_push_no_changes_renders_stages():
    """无待推内容（如仅初始化仓库）：仍正常执行，阶段行末行为结果。"""
    ...
    out = app._view
    assert "Push completed" in out.splitlines()[-1]  # 末行=最后结果
