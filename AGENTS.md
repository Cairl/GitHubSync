...
- 导航栏固定三项：`推送` `拉取` `文件`（三项槽位等宽 `_MENU_SLOT`=11 列（最长内容 9 + 两侧留白 2），内容槽内居中；仅选中项括号可见如 `[推送]` + `#636363` 底色紧贴内容、两侧各留 1 格（宽 = 内容 + 2），未选中项为裸文本无括号；框选左右移动、`*` 同步标记增减只改槽内留白，其他选项位置零偏移，行总宽恒 33，分叉时不再切换为恢复/强制推送）；推送 / 拉取有新的同步时文本两侧加 `*`（如 `*推送*`，选中时 `[*推送*]`，由 `_has_sync()` 判定：CHANGED/AHEAD/NO_REPO/NO_REMOTE/DIVERGED 标记推送，BEHIND/DIVERGED 标记拉取）
